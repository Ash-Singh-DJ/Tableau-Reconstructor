...
description: >-
  End-to-end migration of a Tableau .twbx workbook from AWS Athena (Presto SQL)
  to Snowflake (ANSI SQL). Extracts each datasource's Custom SQL and calculated
  fields, translates the SQL to Snowflake, deploys it as a Snowflake gold view
  (reusing an existing gold table if one already represents that datasource),
  then rewrites the workbook's data connections to point at Snowflake while
  preserving every caption, drill-down hierarchy, and calculated field. Produces
  a new Snowflake-backed .twbx. Use when the user wants to convert an Athena-backed
  Tableau dashboard to Snowflake or "source-swap"/migrate a workbook. Operates in
  the Tableau-Reconstructor repo.
---

# Tableau Source Swap (Athena → Snowflake)

Given an Athena-backed Tableau `.twbx`, produce a Snowflake-backed `.twbx`. For
each SQL datasource the skill extracts its Custom SQL and fields, ensures a
Snowflake gold view exists (reuse or create), then rewrites the workbook's data
connection in place to point at that view — keeping every caption, drill-down
hierarchy, calculated field, and worksheet binding intact.
...
1. Path to the source `.twbx` (Athena, usually extract-backed) — typically under
   `Inputs/`.
...
each datasource (by caption) to its gold view:
...
confirm it's not in a target.

## Output & handoff

The produced workbook connects **live** to Snowflake (extracts are stripped;
offline `.hyper` regeneration isn't possible). The real gate is the **Tableau open
test**, which only the user can run:

1. Open the `Source Swap` `.twbx` (prompts for Snowflake OAuth sign-in).
...
`.twbx` rewrite and are easy to forget:
