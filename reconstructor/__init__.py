"""reconstructor -- format-agnostic Tableau .twbx/.tdsx swap engines (pure stdlib).
...
Plus extraction helpers used to inspect a workbook/data source before swapping:
...
data source (.tdsx, root <datasource>); the format differences are centralized in
  - tableau_doc.py                 read/parse/enumerate/match across .twbx and .tdsx
